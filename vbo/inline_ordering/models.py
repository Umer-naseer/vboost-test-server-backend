...
class Orderable(models.Model):
    ...
    inline_ordering_position = models.IntegerField(
        blank=True,
        default=0,
        null=True,
        editable=True,
    ...
    def save(self, force_insert=False, force_update=False, using=None):
        ...
        if not self.inline_ordering_position:
            max = self.__class__.objects.filter()\
                .aggregate(models.Max('inline_ordering_position'))
            try:
                self.inline_ordering_position = \
                    max['inline_ordering_position__max'] + 1
            except TypeError:
                self.inline_ordering_position = 1
        return super(Orderable, self)\

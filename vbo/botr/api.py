...
class ApiClientException(Exception):
    ...
    def __repr__(self):
        return "%s(*%s)" % \
               (self.__class__.__name__, repr(self.args) if self.args else "")
...
class API(object):
    ...
    def __init__(self, key, secret, protocol='http', host=None, port=None,
        ...
        self._secret = secret
        self._url = '%s://%s%s/%s' % \
                    (protocol, host, ':%s' % port if port else '', version)
        self._kit_version = u'py-%s%s' % \
                            (__version__, '-%s' %  client if client else '')
    ...
    def call(self, api_call, params=dict(), verbose=False):
        ...
        if not isinstance(params, types.DictType):
            raise TypeError(
                "'params' argument must be a dict, not %s" %
        ...
        return self._execute(
            url, response_format=params['api_format'], verbose=verbose
        )
    ...
    def upload(self, params, file_path, verbose=False):
        ...
        return self._execute(
            url, file_path, response_format=params['query']['api_format'],
            verbose=verbose
        )
    ...
    def _execute(self, request_url, file_path=None, response_format='py',
                 verbose=False):
        """Execute an API call
        ...
        if url_lib.__name__ == 'pycurl':
            ...
            if file_path:
                raise NotImplementedError(
                    "File upload is not supported when using %s library" %
                    url_lib.__name__
                )
            urllib2 = url_lib
            output = None
            ...
            if call_failed:
                ...
                try:
                    ...
                    if verbose:
                        ...
                        if (response_content_type.find('application/python')
                                >= 0):
